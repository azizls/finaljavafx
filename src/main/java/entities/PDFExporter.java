...
public class PDFExporter {
    ...
    public static void exportTableViewToPDF(TableView<Panier> tableView, String filePath) {
        try (PDDocument document = new PDDocument()) {
            ...
            for (TableColumn<Panier, ?> column : tableView.getColumns()) {
            ...
            ObservableList<Panier> items = tableView.getItems();
            for (Panier user : items) {
                startX = 50; // Reset X position
                for (TableColumn<Panier, ?> column : tableView.getColumns()) {
